...
namespace DAL
{
    ...
    /// 数据访问类 Show_User
    /// <summary>
    public partial class Show_User : DALHelper
    {
        /// <summary>
        /// 增加一条数据
        /// </summary>
        ...
        {
            IDbDataParameter[] parms4Show_User = PrepareAddParameters(model);
            return dbHelper.ExecuteNonQuery(CommandType.Text, COMMAND_ADD, parms4Show_User);
        }
        ...
        {
            IDbDataParameter[] parms4Show_User = PrepareUpdateParameters(model);
            return dbHelper.ExecuteNonQuery(CommandType.Text, COMMAND_UPDATE, parms4Show_User);
        }
        ...
        {
            IDbDataParameter[] parms4Show_User = PrepareDeleteParameters(Id);
            return dbHelper.ExecuteNonQuery(CommandType.Text, COMMAND_DELETE, parms4Show_User);
        }
        ...
        public DataTable GetDataTable(string strWhere = "", string strOrder = "")
        {
            ...
            strSql.Append("SELECT * FROM Show_User");
            if (strWhere.Trim() != "")
            {
        ...
        {
            IDbDataParameter[] parms4Show_User = PrepareGetModelParameters(Id);
            using (IDataReader dr = dbHelper.ExecuteReader(CommandType.Text, COMMAND_GETMODEL, parms4Show_User))
            {
        ...
        {
            IDbDataParameter[] parms4Show_User = PrepareExistParameters(Id);
            object obj = dbHelper.ExecuteScalar(CommandType.Text, COMMAND_EXISTS, parms4Show_User);
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("SELECT COUNT(*) FROM Show_User");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere);
            }
        ...
        public List<Model.Show_User> GetList(string strWhere = "",string strOrder = "")
        {
            ...
            {
                List<Model.Show_User> lst = new List<Model.Show_User>();
        ...
        public Model.PageData GetPageList(int pageSize, int curPage, string strWhere, string strOrder)
        {
            ...
            List<Model.Show_User> list = new List<Model.Show_User>();
            using (IDataReader dr = dbHelper.ExecuteReader(CommandType.Text, strSql.ToString()))
            {
                ...
                while (dr.Read())
                {
                    ...
                    {
                        Model.Show_User md = new Model.Show_User();
                        PrepareModel(md, dr);
                        list.Add(md);
                    }
                }
        ...
        public List<Model.Show_User> GetListByPage(string strWhere, string strOrder, int startIndex, int endIndex)
        {
            ...
            strSql.Append(")AS Row, T.*  from Show_User T ");
        ...
        /// 由一行数据得到一个实体
        /// </summary>
        private Model.Show_User GetModel(IDataReader dr)
        {
            Model.Show_User model = new Model.Show_User();
